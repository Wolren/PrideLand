...
public class CustomBedBlockEntityRenderer implements BlockEntityRenderer<CustomBedBlockEntity> {
    ...
    @Override
    public void render(CustomBedBlockEntity customBedBlockEntity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j, Vec3d cameraPos) {
    ...
    private void renderPart(MatrixStack matrices, VertexConsumerProvider vertexConsumers, ModelPart part, Direction direction, SpriteIdentifier sprite, int light, int overlay, boolean isFoot) {
        ...
        matrices.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(180.0f + Direction.getHorizontalDegreesOrThrow(direction)));
        matrices.translate(-0.5f, -0.5f, -0.5f);
