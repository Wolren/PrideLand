...
public class RainbowCraftingScreenHandler extends ScreenHandler {
    ...
    void populateResult() {
        if (!RainbowCraftingScreenHandler.this.dyeSlot.getStack().isEmpty() && !this.availableRecipes.isEmpty() && this.isInBounds(this.selectedRecipe.get())) {
            RecipeEntry<RainbowCuttingRecipe> rainbowCuttingRecipe = this.availableRecipes.get(this.selectedRecipe.get());
            ItemStack itemStack = rainbowCuttingRecipe.value().craft(this.input, this.world.getRegistryManager());
