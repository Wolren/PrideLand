...
public class RainbowCraftingScreen extends HandledScreen<RainbowCraftingScreenHandler> {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (!slot.hasStack()) {
            drawTexture(matrices, i + slot.x, j + slot.y, this.backgroundWidth, 15, 16, 16);
        }
    ...
    private void renderRecipeIcons(MatrixStack matrices, int x, int y, int scrollOffset) {
        ...
        for(int i = this.scrollOffset; i < scrollOffset && i < this.handler.getAvailableRecipeCount(); ++i) {
            ...
            this.client
                    .getItemRenderer()
                    .renderInGuiWithOverrides(matrices, list.get(i).getOutput(this.client.world.getRegistryManager()), k, m);
        }
    }
